...
export default function ConfirmedStep({
  provider, service, total, reference, when,
}: { provider: Provider; service: Service; total: number; reference: string; when: string }) {
  return (
    <div className="text-center py-6">
      ...
      <p className="text-slate-500 text-sm mb-7" style={{ fontFamily: "'Inter', sans-serif" }}>
        {provider.name} · {service.name}<br /><strong className="text-slate-800">{when}</strong>
      </p>
      <Card className="p-5 text-left mb-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div><div className="font-bold text-sm text-slate-900">{provider.name}</div><div className="text-xs text-slate-500">{when}</div></div>
        </div>
        <div className="space-y-1.5 text-sm">
          <div className="flex justify-between text-slate-600"><span>Ref</span><span className="font-bold text-slate-900">{reference}</span></div>
